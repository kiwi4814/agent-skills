...
## Personal Workflow Override

If the user explicitly asks to follow a unified Agent Skills workflow, treat `~/.agents/skills/<skill-name>/` as the source of truth for personal skills.

In that workflow:
- create the canonical skill under `~/.agents/skills/<skill-name>/`
- treat `~/.claude/skills`, `~/.codex/skills`, and similar tool-specific directories as read entrypoints rather than the canonical home
- after creating or updating the skill, run `askills_link` and `askills_check`
- follow any user-provided language conventions for skill authoring
- when that workflow is explicitly requested, prefer using `agent-skills-manager` for installation, linking, syncing, and verification details
